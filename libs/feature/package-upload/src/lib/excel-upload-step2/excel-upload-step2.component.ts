...
@Component({
  selector: 'fan-id-excel-upload-step2',
  templateUrl: './excel-upload-step2.component.html',
  styleUrls: ['./excel-upload-step2.component.scss']
})
export class ExcelUploadStep2Component implements OnInit,OnDestroy,AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
        ...
        this.http
          ...
          .subscribe((resp) => {
            if (resp.dataList) {
              ...
              this.dataList = resp.dataList.map((data: any) => {
                return {
                  ...data,
                  checked: this.isApplicationChecked(data.customerCardApplication.id),
                };
              });
              this.validateResponse(this.dataList);
      ...
      initComplete: function (settings, json) {
        $('#packageExcelContinueDataTable').wrap("<div class='fan-id-tablewrap'></div>");
      },
      preDrawCallback: () => {
  ...
  processPackageExcelData() {
    const bulkPackageExcelData = JSON.parse(localStorage.getItem("bulkPackageExcelData") as string);
    if(!bulkPackageExcelData) {
      this.router.navigate([this.backUrl]);
      return;
    }
    this.bulkPackageExcelData = bulkPackageExcelData;
    this.refEvent_Code = this.bulkPackageExcelData.RefEvent_Code;
    this.bulkGroupName = this.bulkPackageExcelData.BulkGroupName;
  }
  ...
  setMetadataOfPackage() {
    ...
    this.userCategory = this.metadataService.getRegUserCategories({ languageId, refresh: false })
      .pipe(map(data => data?.find((item) => {
        return item?.code === this.bulkPackageExcelData.RefRegUserCategory_Code;
      })));
    
    this.applicantType = 
      this.bulkPackageExcelData.RefApplicationType_Code == "Other" ? "GCC or International" : "Qatar Resident";
  }

  setRequest() {
    this.request.filter.bulkGroupName = this.bulkGroupName;
    this.request.filter.refEvent_Code = this.refEvent_Code;
    this.request.filter.refOrganization_Id = this.bulkPackageExcelData.RefOrganization_Id;
  ...
  editApplication(obj:any) {
    ...
    this.router.navigate([
      ...
      queryParams: { id:obj.customerCardApplication.id, feature:'package-excel-upload-draft' }
    });
  }
  ...
  onSubmit() {
    ...
    const organizationId = this.bulkPackageExcelData.RefOrganization_Id;
  ...
  deleteSelectedApplications() {
    ...
    if (data.length > 0 || isSubmitAll) {
      ...
      const payload = {
        data,
        organizationId: this.bulkPackageExcelData.RefOrganization_Id,
        bulkGroupName: this.bulkPackageExcelData.BulkGroupName,
        tournamentCode: this.bulkPackageExcelData.RefEvent_Code,
        isSubmitAll,
  ...
  exportFailedApplication() {
    const body = {
      refOrganization_Id: this.bulkPackageExcelData.RefOrganization_Id,
      bulkGroupName: this.bulkPackageExcelData.BulkGroupName,
      RefTournament_Code: this.bulkPackageExcelData.RefEvent_Code
    }
    this.packageUploadService.exportFailedApplicationsFromPackageExcel(body)
      .pipe(map(data => {
      ...
      .subscribe({
        next: (data) => {
          downloadFile({
            ...
            filename: "ExportedPackage_" + this.bulkPackageExcelData.BulkGroupName + ".xlsx"
          });
        }, error: (err) => {
          console.log(err);
