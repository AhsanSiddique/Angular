...
@Component({
  selector: 'fan-id-approve-application-edit',
  templateUrl: './approve-application-edit.component.html',
  styleUrls: ['./approve-application-edit.component.scss']
})
export class ApproveApplicationEditComponent implements OnInit, OnDestroy {
  ...
  setTicketFormValue(
    ...
    if (this.applicationType === 'FWC') {
      ...
      // const disableTicketForm = !enabledApplicationStatusList.includes(this.applicant_data.applicationStatus);
  ...
  redirectToList() {
    this.router.navigate(['main/approve-application']);
  }
  ...
  closeConfirmApproveModal(event: 'yes' | 'no') {
    this.showApproveConfirmModal = false;
    event === 'yes' && this.approveApplication();
  }

  approveApplication() {
    const body = {
      FANID: this.fanId,
      DocumentIdNo: this.uf.document_number.value,
      applicationStatus: 'APP'
    }
    this.registrationFormService.applicant.updateGCCAccommodationStatus(body)
      .pipe(catchError(err => {
        console.log({ err });
        return EMPTY
      }))
