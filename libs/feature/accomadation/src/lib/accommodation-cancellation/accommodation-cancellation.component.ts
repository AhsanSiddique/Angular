...
import { IAccommodationCancellationGetListRequest, IAccommodationCancellationGetListRequestFilter, IAccommodationCancellationListResponseObject } from '@fan-id/api/server';
import { Environment, FanIDConfig } from '@fan-id/core';
import { DataTableDirective } from 'angular-datatables';
import { of, Subject } from 'rxjs';
import { catchError } from 'rxjs/operators';
import { dataColumns as cancellationDataColumns } from './cancellation.columns';
import { EFilterState } from '../accommodation.common';
import { AccommodationTableBase } from '../accommodation-table.base';

const SessionStorageKeys = {
  FILTER_STATE: 'acc_cancellation_filter_state',
  FILTER_COLUMN: 'acc_cancellation_filter_column',
  FILTER_VALUE: 'acc_cancellation_filter_value',
  FILTER_ADVANCED: 'acc_cancellation_filter_advanced',
  FILTER_ADVANCED_FORM: 'acc_cancellation_filter_advanced_form',
}

@Component({
  selector: 'fan-id-accommodation-cancellation',
  templateUrl: './accommodation-cancellation.component.html',
  styleUrls: ['./accommodation-cancellation.component.scss']
})
export class AccommodationCancellationComponent extends AccommodationTableBase implements OnInit, AfterViewInit, OnDestroy {
  @Input() event:any;
  ...
  indexSaveBool = false;
  indexSave = 0;
  filter: IAccommodationCancellationGetListRequest = {};
  dataList: IAccommodationCancellationListResponseObject[] = [];
  dataColumns = cancellationDataColumns;
  tableColumns = [
    ...(cancellationDataColumns.map(({ title, dataKey }) => ({title, value: dataKey})))
  ]
  filterColumns = cancellationDataColumns.filter(({ filter }) => filter);
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.http
          .post<any>(
            this.apiUrl+'/api/Accommodation/AccommodationCancellationList',
            {
              ...this.filter,
              pageIndex:
      ...
      initComplete: function (settings, json) {
        $('#cancellation-datatable').wrap("<div class='fan-id-tablewrap'></div>");
      },
      preDrawCallback: () => {
  ...
  ngAfterViewInit() {
    this.filter.filter = this.getSavedFilter() as IAccommodationCancellationGetListRequestFilter;
