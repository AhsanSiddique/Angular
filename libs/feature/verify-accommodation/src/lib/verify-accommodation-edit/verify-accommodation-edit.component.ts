...
import { convertNgbDateToISO, convertDateStringToNgbDate, fromJSDate} from "@fan-id/shared/utils/date";
...
@Component({
  selector: 'fan-id-verify-accommodation-edit',
  templateUrl: './verify-accommodation-edit.component.html',
  styleUrls: ['./verify-accommodation-edit.component.scss']
})
export class VerifyAccommodationEditComponent implements OnInit, OnDestroy {
  ...
  dependentPage = 1;
  verifyAccommodationData: IVerifyAccommodationItem | null = null;
  departureCountryCode!: string;
  accommodationStatusToUpdate: EAccommodationStatus = EAccommodationStatus.Approved;
  ...
  ngOnInit() {
    ...
    this.setVerifyAccommodationData();
  ...
  setVerifyAccommodationData() {
    this.verifyAccommodationData = getFromLocalStorage({key: 'verifyAccommodationData', parse: true}) as IVerifyAccommodationItem;
    this.departureCountryCode = this.verifyAccommodationData?.departureCountryCode ?? null;
  }
  ...
  initializeForms() {
    ...
    this.residencyForm = this.applicantForm.get(
      'residencyForm'
    ) as FormGroup;
    ...
    this.registrationFormService.applicant
      ...
      .subscribe((response) => {
        ...
        this.dependentList = dependentList.map((dependent) => {
          ...
          profilePic = this.registrationFormService.applicant.composeImageUrl(profilePic);
  ...
  async patchOCRFields(response) {
    ...
    const ocr_fields_to_patch = [
      ...
      ...( idExpiryDateStr ? [{ value: document_expiry, field: this.uf.document_expiry }] : []),
  ...
  redirectToList() {
    this.router.navigate(['main/verify-accommodation']);
  }
  ...
  showUpdateStatusModal(status: EAccommodationStatus) {
    this.accommodationStatusToUpdate = status;
    this.isUpdateStatusModalOpen = true;
  }

  async viewDocument() {
    const pdfPath = this.verifyAccommodationData?.localFilePath_Accommodation;
    if (!pdfPath) {
      alert('No document found!');
      return;
    }
    this.pdfSrc = await this.registrationFormService.applicant.getResourceSafeUrl(pdfPath) as string;
