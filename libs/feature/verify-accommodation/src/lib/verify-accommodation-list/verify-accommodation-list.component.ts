...
@Component({
  selector: 'fan-id-verify-accommodation-list',
  templateUrl: './verify-accommodation-list.component.html',
  styleUrls: ['./verify-accommodation-list.component.scss']
})

export class VerifyAccommodationListComponent
  ...
  verifyApplicationForm = new FormGroup({
    ...
    accommodationName: new FormControl(null),
    otherAccommodationCategory_Name: new FormControl(null),
    accommodationStartDate: new FormControl(null),
    accommodationEndDate: new FormControl(null),
    departureCountry_Name: new FormControl(null),
    hayyaNo: new FormControl(null),
    dateOfSubmitted: new FormControl(null),
  });
  ...
  filterColumnsSearch: {
    ...
    //   value: 'otherAccommodationCategory_Name'
    // },
    {
      id: 12,
      name: 'Accommodation Start Date',
      value: 'accommodationStartDate',
      type: SearchType.Date,
    },
    {
      id: 13,
      name: 'Accommodation End Date',
  ...
  applicationStatusList$: Observable<{value: number, name: string}[]>;
  ...
  constructor(
    ...
    this.applicationStatusList$ = this.metadataService.getApplicationStatuses();
  }
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.http
          .post<any>(
            this.config.apiUrl +
              '/api/OtherAccommodation/getList',
            {
              ...this.filter,
              pageIndex:
      ...
      columns: [
        ...
        {
          data: 'otherAccommodationCategory_Name',
        },
        {
          data: 'accommodationStartDate',
        },
        {
          data: 'accommodationEndDate',
        },
        {
          data: 'departureCountry_Name',
        }
      ],
      //**************** */
      initComplete: function (settings, json) {
        $('#verifyAccommodationdatatable').wrap(
  ...
  filterLoadingInitial() {
    if (sessionStorage.getItem('verifyAccomColumnName')) {
      this.storedSelectColumn = parseInt(
        sessionStorage?.getItem('verifyAccomColumnName')
      );
    ...
    const filterTxt = JSON.parse(
      sessionStorage?.getItem('verifyAccomFilterTxt')
    );
    if (filterTxt) {
    ...
    if (
      ...
      if (sessionStorage.getItem('advVerifyAccomFilters')) {
        this.advFilterStored = JSON.parse(
          sessionStorage.getItem('advVerifyAccomFilters')
        );
        if (this.advFilterStored) {
          this.advancedFilterBoolean = true;
          this.advFilterStored.forEach((item) => {
            const dateFilterKeys = ['accommodationStartDate', 'accommodationEndDate', 'dateOfSubmitted', 'appdate'];
            if (dateFilterKeys.includes(item?.controlname)) {
  ...
  clearFilter() {
    ...
    sessionStorage.removeItem('verifyAccomColumnName');
    sessionStorage.removeItem('verifyAccomFilterTxt');
    sessionStorage.removeItem('advVerifyAccomFilters');
    this.arrayOffilters = [];
    localStorage.removeItem(
      'DataTables_verifyAccommodationdatatable_/main/verify-application'
    );
  ...
  setAllApplicationsFilter() {
    return {
      ...
      ...(this.isServiceCenter && {
        includeSaved: false,
        otherAccommodationStatus: 1,
        applicationStatus: ApplicationStatus.Pending_Accommodation_Confirmation
      }),
    };
  }
  public filterColumn() {
  ...
  verifyApplication(data) {
    localStorage.setItem("verifyAccommodationData",JSON.stringify(data));
    this.router.navigate(['main/verify-accommodation/edit'], {
  ...
  setAdvancedSearchFiltersText() {
    ...
    if (
      !this.aps.isNullOrEmpty(this.verifyApplicationForm?.get('hayyaNo')?.value)
    ) {
      this.filter.filter['hayyaNo'] =
        '#' + this.verifyApplicationForm?.get('hayyaNo')?.value + '#';
      this.arrayOffilters.push({
        col: 'hayyaNo',
        value: this.verifyApplicationForm?.get('hayyaNo')?.value,
        controlname: 'hayyaNo',
        hashed: true,
      });
    }
  ...
  setAccommodationFilter() {
    const accommodationAdvancedFilters: {name: keyof IVerifyAccommodationItem, type: SearchType}[] = [
      { name: 'accommodationName', type: SearchType.Text },
      { name: 'otherAccommodationCategory_Name', type: SearchType.Text },
      { name: 'departureCountry_Name', type: SearchType.Text },
      { name: 'accommodationStartDate', type: SearchType.Date },
      { name: 'accommodationEndDate', type: SearchType.Date },
      { name: 'dateOfSubmitted', type: SearchType.Date },
    ]
    accommodationAdvancedFilters.forEach(({name, type}) => {
      if (
        !this.aps.isNullOrEmpty(
          this.verifyApplicationForm?.get(name)?.value
        )
      ) {
        let value = this.verifyApplicationForm?.get(name)?.value;
        if (type === SearchType.Text) {
          this.filter.filter[name] =
            '#' + value + '#';
        }
        if (type === SearchType.Date) {
          value = convertNgbDateToDDMMYYYY(value);
