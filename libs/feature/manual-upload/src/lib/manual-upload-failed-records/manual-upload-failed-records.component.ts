...
import { AccreditationForm, ApplicationFormComponent, UploadDocumentFormComponent } from "@fan-id/shared/shared-forms";
import { DIPLOMAT_QID_EXPIRY_DATE, checkPassportDateOfBirthBelowYear, convertDateDDMMYYYYToMMDDYYYY, convertDateStringToNgbDate, convertNgbDateToISO, fromJSDate } from "@fan-id/shared/utils/date";
import { CATEGORIES_WITH_GCC_PERMIT_SC, EPassportCategory, FORM_PLACEHOLDERS, IFormExtras, MIN_FACE_MATCH_SCORE, TicketTypes, getB2BVisaType, getFormExtras, getPassportOCRCategory, isCountryGCC, onUniqueFieldBlur, patchOCRfield, resetPatchedOCRfield, updateFormExtras } from '@fan-id/shared/utils/form';
import { NgbDateStruct } from "@ng-bootstrap/ng-bootstrap";
import * as getCountryISO2 from 'country-iso-3-to-2';
import { Observable, Subject, throwError } from "rxjs";
import { catchError, delay, first, map, take, tap } from "rxjs/operators";
import { ManualStepOneForm } from "../manual-upload-step1/manual-upload-step1.component";
@Component({
  selector: 'fan-id-manual-upload-failed-records',
  templateUrl: './manual-upload-failed-records.component.html',
  styleUrls: ['./manual-upload-failed-records.component.scss']
})
export class ManualUploadFailedRecordsComponent extends AccreditationForm implements OnInit, OnDestroy, AfterViewInit {
  constructor(
  ...
  deleteApplicant(confirm = false) {
    ...
    const applicantData = this.applicants.find(applicant => applicant?.id === this.selectedApplicantId)

    if (applicantData && applicantData.data) {
      const { data } = applicantData;
      const { id, system_RowVersion } = data;
      this.registrationFormService.applicant.deleteApplicantFromBulkDraft({ id, system_RowVersion })
        .pipe(take(1))
        .subscribe(response => {
          console.log({ response })
          this.applicants = [
            ...(
              this.applicants.filter(applicant => applicant?.id !== applicantData.id)
                .map((applicant, index) => {
          ...
          this.currentApplicantId = this.currentApplicantId !== 1 ? --this.currentApplicantId : 1;
  ...
  getApplicationInsertBody() {
    ...
    EmergencyContactOneFullName =
      EmergencyContactOneFullName.replace(/[^a-zA-Z]/g, ' ').trim().split(' ')[0].slice(0, 20)
  ...
  async submitForm({ addApplicant, save }: { addApplicant?:boolean, save?:boolean } = {}) {
    ...
    this.registrationFormService.applicant
      ...
      .subscribe(
        ...
        (err) => {
          ...
          this.register_error_message = err?.error?.message || "Something went wrong!";
        }
      );
  }
  ...
  onSubmit() {
    ...
    const totalSubmissions = this.applicants.length - 1;
    this.processingdata = { totalCount: totalSubmissions };
    const data = this.applicants.filter(applicant => !!applicant?.data?.id)
  ...
  get selectedApplicant() {
    return this.applicants.find(applicant => applicant?.id === this.selectedApplicantId)
  }
  ...
  addCurrentApplicantToList(applicantDataFromDraft = null, isApplicationEdit = false) {
    const id = isApplicationEdit ? this.selectedApplicantId : this.applicants.length;
  ...
  async onApplicantSelect(applicantId) {
    console.count('onApplicantSelect')
    const applicantData = this.applicants.find(applicant => applicant?.id === applicantId)
    this.clearData();
    this.registrationFormService.scroll.scrollToTop();
    this.currentApplicantId = this.selectedApplicantId = applicantId;
    console.log({applicantId, applicantData})
    if(applicantData) {
      const { formValue, continue: _continue, name, bulk_group_continue, data } = applicantData;
      ...
      if (uploadDocumentFormValue.document_proof) {
  ...
  onRegisterError(event: any) {
    ...
    event === 'close' || this.redirectToStepOne();
  }
